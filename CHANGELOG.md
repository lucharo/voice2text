...
- **Transcription runs while the hotkey is held.** With the Parakeet backend the recording is fed to the recogniser in 5 s pushes as it happens, so on release only the last push is left to decode (~0.3 s on an M4 Pro) instead of ~11 ms per second of audio; the log and the menu-bar tooltip show how many words it has heard so far. `streaming = false` under `[transcription]` restores whole-file decoding, which Whisper always uses (#12).
